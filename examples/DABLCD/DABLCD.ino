...
{
  //new text...but we compare to the previous text, as an update isn't always different next.
  if (strcmp(service_text, Dab.ServiceData) != 0)
  {
    ...
    replaceSpecialCharsInplace(service_text, strlen(service_text));
    //remove spaces from end of string.
    for (uint8_t i = strlen(service_text) - 1; i > 0; i--)
    {
      if (service_text[i] == ' ')
      {
        service_text[i] = '\0';
...
void process_buttons(uint8_t buttons)
{
  ...
  {
    menu_timer = 2000;  //stay in menu mode
    if (buttons & BUTTON_RIGHT)
    {
      if (menu_mode < 1)
      {
