/*
 * DAB Slideshow Example
 * DAB Radio with Colour LCD, and Slideshow images
 * AVIT Research Ltd
 *
 * Platform:
 *    WeMos D1 R32 ESP32 
 ...
 *  o Tjpg_Decpder (v1.1.0)
 *
 * DABSlideshow.ino
 * Main Code File
 * v0.1 01/04/2026 - initial release
  */
...
//https://imagecolorpicker.com/color-code/83DD31
#define COLOR_BACKGROUND RGB565(0x426f19)     //50% shade
#define COLOR_ENSEMBLE_TEXT RGB565(0x76c72c)  //20% shade
#define COLOR_SERVICE_TEXT RGB565(0x83DD31)   //00% tint

//https://imagecolorpicker.com/color-code83DD31
#define COLOR_DATE_TIME RGB565(0x83DD31)  //00% tint
...
#define MAX_IMAGE_SIZE	(48 * 1024)	//48k bytes
...
// Must be put back to #if 0 otherwise will reformat every boot.
#if 0
  SPIFFS.format();
#endif
  ...
  Dab.slideshow(imageData, MAX_IMAGE_SIZE, DisplaySlideShow);
  Dab.begin(0);
...
void Display_Boot_Screen(void) {
  ...
  displayText(240, 120, "AVIT SlideShow Radio", COLOR_BACKGROUND, COLOR_RGB565_WHITE, 1, JUSTIFIED_CENTER);
  displayText(240, 140, "v1.0.0 2026", COLOR_BACKGROUND, COLOR_RGB565_WHITE, 1, JUSTIFIED_CENTER);
...
void Display_DAB_Screen(void) {
  ...
  //Ensemble
  displayText(190, 20, Ensemble[ensemble].Ensemble, COLOR_ENSEMBLE_TEXT, COLOR_BACKGROUND, 1, JUSTIFIED_LEFT);
  //Service
  displayText(190, 40, Ensemble[ensemble].service[service].Label, COLOR_SERVICE_TEXT, COLOR_BACKGROUND, 1, JUSTIFIED_LEFT);
}
...
bool tft_render(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t* bitmap)
{
  // Limit image to 160x120 from (20,20)
  if ( x >= 20+160 ) return 1;  //continue decoding
  if ( y >= 20+120 ) return 0;  //end decoding
...
{				
  if(type == DAB_JPEG)
  {
    ...
    TJpgDec.setCallback(tft_render);
...
void DAB_NextService(void) {
  if (Ensemble[ensemble].numberofservices > 0) {
    ...
    tft.fillRect(20, 20, 160, 120, COLOR_RGB565_BLACK);
    tft.fillRect(190, 20, 195, 40, COLOR_BACKGROUND);
    //Ensemble
    displayText(190, 20, Ensemble[ensemble].Ensemble, COLOR_ENSEMBLE_TEXT, COLOR_BACKGROUND, 1, JUSTIFIED_LEFT);
    //Service
    displayText(190, 40, Ensemble[ensemble].service[service].Label, COLOR_SERVICE_TEXT, COLOR_BACKGROUND, 1, JUSTIFIED_LEFT);
